...
def load_geojson_to_dataframe(path: str) -> pd.DataFrame:
    ...
    for feature in data["features"]:
        ...
        seen: set[tuple[str, str]] = set() # for avoiding duplicates

        for i in range(1, 9):
            code = props.get(f"lm{i}")
            text = props.get(f"lm{i}_tekst")

            # if the sign code is not relevant
...
def remove_leading_distances(chunk: str) -> str:
    ...
    while True:
        # stop if it already starts with a letter
...
def normalize_separators(text: str) -> str:
    ...
    # unify common separators to semicolon
    text = text.replace("/", ";")
    text = text.replace(",", ";")
...
def main() -> None:
    ...
    fixable_df = flat_df[
        (~flat_df["is_clean_raw"]) & (flat_df["is_clean_after_fix"])
        ].copy()
    ...
    final_clean_df = pd.concat(
        [clean_df, fixable_df, usable_messy_df],
        ignore_index=True
    )

    final_messy_df = dropped_messy_df.copy()

    messy_with_numbers_df = final_messy_df[
        final_messy_df["text"].apply(has_any_distance)
    ].copy()

    messy_without_numbers_df = final_messy_df[
        ~final_messy_df["text"].apply(has_any_distance)
    ].copy()
    ...
    print("\n=== Already clean ===")
    print(clean_df[["text"]].head(10))

    print("\n=== Fixed by normalization ===")
    print(fixable_df[["text", "normalized_text"]].head(10))

    print("\n=== Still messy ===")
    print(messy_df[["text"]].head(20))

    print("\n=== Usable messy after chunk filtering ===")
    print(usable_messy_df[["text", "cleaned_text"]].head(20))

    print("\n=== Dropped messy signs ===")
    print(dropped_messy_df[["text"]].head(20))

    print("\n=== Sample clean signs ===")
    print(clean_df[["code", "text"]].head(20))

    print("\n=== Sample messy signs ===")
    print(messy_df[["code", "text"]].head(20))
    ...
    print("\n=== Sample FINAL CLEAN ===")
    print(final_clean_df[["code", "text", "final_text"]].sample(20))

    print("\n=== Sample FINAL MESSY ===")
    print(final_messy_df[["code", "text"]].sample(20))
    ...
    print("\n=== Sample messy WITH numbers ===")
    print(messy_with_numbers_df[["code", "text"]].sample(20))

    print("\n=== Sample dropped (no distance) ===")
    print(messy_without_numbers_df[["code", "text"]].sample(20))
